...
def robotControl():
    global ZB
    global joystick
    # not sure if global declarations below are needed for servo & laser. 
    # if needed, cool. if not needed, delete the global declaration below
        ...
        while running:
            ...
            for event in events:
                ...
                if hadEvent:
                    ...
                    # depending on your configuration, you may need to exchange the -ve signs position
                    if joystick.get_button(axisL2):
                        # to move BACKWARD
                        driveLeft = throttle
                        driveRight = throttle
                    else:
                    ...
                    tilt = -joystick.get_axis(tilt_axis)
                    pan = -joystick.get_axis(pan_axis)
                    
                    tilt_servo.value = tilt
                    pan_servo.value = pan
                    
                    if joystick.get_button(laser_button):
                        laser.toggle()
